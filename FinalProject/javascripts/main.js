//Renderer Elements
var context = document.body.appendChild(document.createElement('canvas')).getContext('2d'),
...
function resize() {
  ...
  if (preHeight <= window.innerHeight) {
    ...
    context.canvas.height = preHeight;
  } else {
    ...
    context.canvas.height = newWidth / ratio;
  }

  renderer.domElement.style.width = '';
  renderer.domElement.style.height = '';
  renderer.domElement.style.left = context.canvas.style.left = (window.innerWidth - renderer.domElement.width) / 2 + 'px';
  renderer.domElement.style.top = context.canvas.style.top = (window.innerHeight - renderer.domElement.height) / 2 + 'px';
}
...
star.castShadow = false; // Cause its a light source
scene.add(star);
...
function animate() {
  ...
  for (var s in bgStars) {
    var q = bgStars[s],
      oX = q.x * context.canvas.width,
      oY = q.y * context.canvas.height,
      size = Math.random() < .9998 ? Math.random() : Math.random() * 3;

    context.beginPath();
    context.moveTo(oX, oY - size);
    context.lineTo(oX + size, oY);
    context.lineTo(oX, oY + size);
    context.lineTo(oX - size, oY);
    context.closePath();
    context.fill();
  }
